...
class HerosNet(nn.Module):
    def __init__(self, Ch, stages, size0 = 288, size1 = 400):
        ...
        ## Mask Initialization ##
        self.Phi = Parameter(torch.ones(self.size0, self.size1), requires_grad=True)
    ...
    def forward(self, training_label):
        ...
        PhiWeight = Phi_.contiguous().view(1,1,training_label.shape[2],training_label.shape[3])
        PhiWeight = PhiWeight.repeat(batch,self.Ch,1,1)

        temp = training_label.mul(PhiWeight)
        temp_shift = torch.Tensor(np.zeros((batch, self.Ch, self.size0, self.size1 + (self.Ch - 1) * 2))).cuda()
        temp_shift[:,:,:, 0:self.size1] = temp
