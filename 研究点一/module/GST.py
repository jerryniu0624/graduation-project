...
class GST_MODEL(nn.Module):
    def __init__(self,
        ...
        m_head = [default_conv(in_ch, n_feats, kernel_size)]

        m_noise_body = [Stack_Block(default_conv, n_feats_noise, kernel_size, act=act, res_scale=1),Res_GCN_BCP(n_feats_noise, inter_channels, spatial_scale)]
        m_noise_body.append(act)
        m_noise_body.append(default_conv(n_feats_noise, 1, kernel_size,))
        m_noise_body.append(noise_act)

        m_body = [ResBlock(default_conv, n_feats, kernel_size, act=act, res_scale= 1) for _ in range(n_resblocks)]
        m_body.append(default_conv(n_feats, n_feats, kernel_size))

        m_tail = [default_conv(n_feats, out_ch, kernel_size)]
