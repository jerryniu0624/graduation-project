...
class MS_MSA(nn.Module):
    def __init__(
        ...
        self.to_q = nn.Linear(dim, int(dim_head * heads), bias=False)
        self.to_k = nn.Linear(dim, int(dim_head * heads), bias=False)
        self.to_v = nn.Linear(dim, int(dim_head * heads), bias=False)
        self.rescale = nn.Parameter(torch.ones(heads, 1, 1))
        self.proj = nn.Linear(int(dim_head * heads), dim, bias=True)
...
class MST(nn.Module):
    def __init__(self, in_dim=50, out_dim=50, dim=50, stage=2, num_blocks=[2,4,4]):
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=103, n_feat=103, stage=3):
