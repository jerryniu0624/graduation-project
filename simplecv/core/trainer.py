...
class Launcher(object):
    ...
    def compute_loss_gradient(self, data, selection=None, segments=None):
        """
        ...
        for d in data:
            ...
            if 'test' in self._model_dir :
                msg_dict = self._model(*d, selection)
            elif 'SS' in self._model_dir : # Semi_Supervised
                if segments is not None:
                    segments = tensor_util.to_device(torch.tensor(segments), self._device)
                msg_dict = self._model(*d, segments)
            else:
    ...
    def train_iters(self, train_data_loader, test_data_loader=None, selection=None,
                    **kwargs):
        ...
        segments = train_data_loader.dataset.segments
        ...
        while self._ckpt.global_step < num_iters:
            ...
            self._model.train()
            loss_dict = self.compute_loss_gradient(data_list, selection, segments)
    ...
    def train_by_config(self, train_data_loader, config, test_data_loader=None, selection=None):
        self._training = True
        ...
        if 'num_epochs' in config and 'num_iters' not in config:
            ...
            signal_loss_dict = self.train_iters(train_data_loader, test_data_loader=test_data_loader, selection=selection,  **config)
