...
def run(config_path, model_dir, cpu_mode=False, after_construct_launcher_callbacks=None, opts=None, np_seed=None, selection=None):
    # 0. config
    ...
    # 2. data
    traindata_loader = make_dataloader(cfg['data']['train'], np_seed)
    testdata_loader = make_dataloader(cfg['data']['test'], np_seed) if 'test' in cfg['data'] else None
    ...
    tl.train_by_config(traindata_loader, config=merge_dict(cfg['train'], cfg['test']), test_data_loader=testdata_loader, selection=selection)
