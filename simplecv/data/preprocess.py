...
def intact_divisible_pad(image_list, size_divisor=128, to_tensor=True):
    """
    ...
    max_shape[2] = int(np.ceil(max_shape[2] / size_divisor) * size_divisor)
    max_shape[3] = int(np.ceil(max_shape[3] / size_divisor) * size_divisor)
    ...
    if to_tensor:
        ...
        out = np.zeros([len(image_list), max_shape[0], max_shape[1], max_shape[2], max_shape[3]], np.float32)

    for i, resized_im in enumerate(image_list):
        out[i, :, :, 0:resized_im.shape[2], 0:resized_im.shape[3]] = torch.from_numpy(resized_im)
